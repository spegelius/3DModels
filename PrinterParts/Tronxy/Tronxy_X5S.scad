...
module tronxy_corner_bearing_mount_left() {
    ...
    difference() {
        union() {
            ...
            translate([-5, 0, -20])
            chamfered_cube(10, 35, h, 4);

            translate([0, -5, -20])
            chamfered_cube(35, 10, h, 4);
        }
        translate([34, 10, 0]) {
            cylinder(d=4.3, h=h*3, center=true, $fn=40);

            translate([0, 0, 5])
            cylinder(d=8.5, h=h, $fn=50);
        }
        translate([18, 10, 0]) {
            cylinder(d=4.3, h=h*3, center=true, $fn=40);
...
module gt2_idler_625zz() {
    ...
    union() {
        difference() {
            union() {
                hull() {
                    cylinder(d=d + 4, h=1, $fn=80);
                    cylinder(d=d + 3, h=1.5, $fn=80);
                }
                cylinder(d=d, h=h - 1.5, $fn=100);

                translate([0, 0, h - 1.5])
                cylinder(d1=d - 5, d2=d - 2, h=1.5, $fn=80);
            }
            cylinder(d=16 + 0.5, h=h*3, $fn=80);
            for(i = [0:3]) {
                rotate([0, 0, 360/4*i])
                translate([(d - 5)/2 + 2/2, 0, h - 1.5 + 2/2])
...
module gt2_idler_625zz_retainer() {
    ...
    union() {
        ...
        difference() {
            for (i = [0:3]) {
                rotate([0, 0, i*90 + 45])
                translate([(d - 4.8)/2 + 2/2, 0, 1.5/2])
                cube([2, 5.5, 1.5], center=true);
            }
