...
module main_body(crevices=false) {
    ...
    module form(w, d, l, h) {
        ...
        module subform() {
            rotate([90,0,0]) {
                hull() {
                    translate([1,0,0]) cylinder(d=d,h=l,center=true,$fn=40);
                    translate([-d,d/2-0.5,0]) cube([1,2*d,l],center=true);
                    translate([0,-d/2-0.5,0]) cube([d,1,l],center=true);
                }
                hull() {
                    translate([1,0,4]) sphere(d=d+3,$f=50);
                    translate([1,0,-4]) sphere(d=d+3,$f=50);
                }
            }
        }
        
        translate([-w/2,0,0]) subform();
        translate([w/2,0,0]) mirror([1,0,0]) subform();
    }
    
    difference() {
        difference() {
            translate([0,0,height/2]) rounded_cube_side(width,length,height,corner,center=true);
            form(width,12,fastener_l+2);
        }
        if (crevices == true) {
            difference() {
                translate([0,0,inner_h/2+2.5]) rounded_cube(inner_w, inner_l, inner_h, inner_corner,center=true);
                form(inner_w,12+2*wall,fastener_l+2+2*wall);
            }
        } else {
            difference() {
                translate([0,0,inner_h/2+wall]) rounded_cube(inner_w, inner_l, inner_h, inner_corner,center=true);
...
module box(crevices=false) {
    ...
    union() {
        ...
        mirror([1,0,0]) translate([fastener_x_offset, -fastener_hole_l/2+0.1, height-10]) rotate([0,0,90]) fastener_slot();
        mirror([0,1,0]) mirror([1,0,0]) translate([fastener_x_offset, -fastener_hole_l/2+0.1, height-10]) rotate([0,0,90]) fastener_slot();
    }
}
...
module lid() {
    ...
    union() {
        difference() {
            translate([0,0,(wall+2)/2]) rounded_cube_side(lid_outer_w, lid_outer_l, wall+2, 10+2*wall+0.2, center=true);
            translate([fastener_hole_x, fastener_hole_y, -0.6]) rotate([-90,0,0]) cylinder(d=3,h=fastener_hole_l,$fn=30);
            translate([-fastener_hole_x, fastener_hole_y, -0.6]) rotate([-90,0,0]) cylinder(d=3,h=fastener_hole_l,$fn=30);

            translate([0,0,wall+10/2]) difference() {
            ...
            if (center_corner >= 0.5) {
                translate([0,0,2.1/2+wall]) rounded_cube_side(lid_inner_w,lid_inner_l,2.1, center_corner, center=true);
            } else {
                translate([0,0,2.1/2+wall]) cube([lid_inner_w,lid_inner_l,2.1], center=true);
            }
    ...
    difference() {
        union() {
            ...
            difference() {
                ...
                translate([-10,(fastener_dia-1)/2,0]) cube([7,9,fastener_l+1]);
            }
            translate([-8,12.5,0]) cylinder(d=2, h=fastener_l, $fn=20);
            translate([-8.9,4.5,fastener_l/2]) difference() {
...
module view_proper() {
    intersection() {
        union() {
            box();
            translate([0,0,height+wall]) rotate([180,0,0]) lid();
            translate([0,0,height+wall]) box();
        }
