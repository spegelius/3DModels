...
//cable_tray(149);
//cable_tray(190);
//cable_tray(130, top_guides=0, bottom_guides=0);
//cable_tray(180, top_chamfer=-45);
//cable_tray(180, bottom_chamfer=45, support=true);
//cable_tray(200, bottom_chamfer=45, support=true);
//cable_tray(200, top_chamfer=-45);
//cable_tray(300, bottom_guides=0);
//cable_tray_300_x2();
...
module _tray(h=100) {
    ...
    difference() {
        ...
        hull() {
            cube(
                [width - 2*wall, 7.1, h + 10],
                center=true
            );

            translate([0, -1/2 + 7.1/2 + 4, 0])
            cube(
                [width - 2*wall - 15, 1, h + 1],
                center=true
            );
        }
...
module cable_tray(
    h=100, hole_offset=10, top_guides=2,
    bottom_guides=1, bottom_chamfer=0,
    top_chamfer=0, support=false
) {
    ...
    module _support() {
        ...
        // left
        translate(
            [-width/2 + 1/2, -depth/2 + 1, depth/2]
        )
        cube([0.8, 1, depth], center=true);

        translate([
            -width/2 + 1/2, -depth/2 + 3, depth/2
        ])
        cube([0.8, 1, depth], center=true);

        translate([
            -width/2 + 1/2, -depth/2 + 6, depth/2
        ])
        cube([0.8, 1, depth], center=true);

        translate([
            -width/2 + 1/2, -depth/2 + 8.9, depth/2
        ])
        cube([0.8, 1, depth], center=true);
...
module cable_tray_cover(h=100) {
    ...
    difference() {
        rounded_cube_side(
            30, 4, h, 1, center=true, $fn=20
        );

        hull() {
            translate([
                0, (4 - wall - 1.3)/2 - 4/2 + wall
            ])
            cube([
                30 - 2*wall, 4 - wall - 1.3, h + 1
            ], center=true);
