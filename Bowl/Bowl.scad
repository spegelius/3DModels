...
module bowl_rim() {
    translate([0,0,99/2]) translate([0,0,-8]) donut(100-wall-1.3,wall/2+0.4,$fn=resolution);
}

module _bowl_bottom() {
    translate([0,0,99/2]) intersection() {
        translate([0,0,-100/2+4]) donut(53,4,$fn=resolution);
        cylinder(d=105,h=99,center=true);
    }
}
...
module _bowl_2() {
    module _form_2() {
        difference() {
            ...
            translate([0,-30/2-5,0]) cylinder(d=30,h=20,center=true,,$fn=40);
            translate([0,30/2+5,0]) cylinder(d=30,h=20,center=true,,$fn=40);
        }
    }
    ...
    module _form_3() {
        difference() {
            ...
            for(i = [0:3]) {
                rotate([0,0,90*i])
                translate([5,5,0])
                cylinder(d=2.5,h=20,center=true,$fn=20);
            }
        }
    }
