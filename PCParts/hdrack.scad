...
function get_height(disks, spacing, wall) = disks*hd_height + disks*spacing + 2*wall;

function get_hole_z(i,spacing, wall) = i*(hd_height+spacing) + (hd_height-6.40) + wall+spacing/2;
...
module hd_rack(disks, spacing=3, closed=false) {
    ...
    difference() {
        ...
        cube([w, 130, get_height(disks, spacing, wall)]);
        translate([wall,0,wall]) cube([hd_width + 0.2, 160, height-2*wall]);
...
module fan_adapter_92mm(disks, spacing=3) {
    ...
    difference() {
        union() {
            difference() {
                hull() {
                    translate([0,0,1/2]) cube([fan_92mm, fan_92mm, 1], center=true);
                    translate([0,0,h+1/2]) cube([w, l, 8], center=true);
                }
                translate([0,0,2]) scale([(w-3)/w,(l-3)/l,1]) hull() {
                    translate([0,0,1/2]) cube([fan_92mm, fan_92mm, 1], center=true);
                    translate([0,0,h+1/2]) cube([w, l, 8], center=true);
                }
                cylinder(d=90,h=5,$fn=100,center=true);
            }
            translate([w/2-wall/2,l/2-9,h+10]) difference() { 
                cube([wall,12,22],center=true);
                translate([-wall/2,0,5]) rotate([90,0,0]) hoole();
            }
            translate([w/2-wall/2,-l/2+22.5,h+10]) difference() { 
                cube([wall,12,22],center=true);
                translate([-wall/2,0,5]) rotate([90,0,0]) hoole();
            }
        ...
        for (i = [0:3]) {
            rotate([0,0,i*360/4]) translate([fan_92mm_bolt_offset,fan_92mm_bolt_offset]) cylinder(d=5.2,h=6,$fn=30);
        }
