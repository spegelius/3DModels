...
//    );
    ...
    _hd_mocks(width, depth, count=14);
...
module _top_covers(width, depth, height) {
    ...
    rotate([90, 180, 180])
    front_cover_grill(width, 175, 24, 6, brim=false, supports=false);

    translate([width/2 - 15, 35.2 + 175, height - 15])
    rotate([90, 180, 180])
    front_cover_grill(width, 175, 21, 0, brim=false, supports=false);

    translate([width/2 - 15, 35.3 + 350, height - 15])
    rotate([90, 180, 180])
    front_cover_bottom(width, 130);
}